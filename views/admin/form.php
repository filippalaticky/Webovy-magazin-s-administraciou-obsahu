...
<section class="editor-shell fade-in">
    <div class="section-heading">
        ...
        <a class="btn btn-secondary" href="<?= e(app_index_url(['action' => 'admin'])) ?>">Spat do panelu</a>
    </div>
    ...
    <form method="post" action="<?= e((string) $formAction) ?>" class="editor-grid">
        <input type="hidden" name="csrf_token" value="<?= e((string) ($_SESSION['csrf_token'] ?? '')) ?>">

        <label for="title">Nazov</label>
        <input id="title" name="title" type="text" maxlength="180" value="<?= e($titleValue) ?>" required>

        <label for="excerpt">Perex</label>
        <textarea id="excerpt" name="excerpt" rows="3" maxlength="320" placeholder="Krátky text do náhľadu"><?= e($excerptValue) ?></textarea>

        <label for="content">Obsah</label>
        <textarea id="content" name="content" rows="10" maxlength="12000" required><?= e($contentValue) ?></textarea>

        <label for="cover_image_url">URL obrazku</label>
        <input id="cover_image_url" name="cover_image_url" type="url" value="<?= e($coverImageUrlValue) ?>" placeholder="https://...">
        ...
        <div class="actions">
            ...
            <a class="btn btn-secondary" href="<?= e(app_index_url(['action' => 'admin'])) ?>">Zrusit</a>
        </div>
    </form>
</section>
